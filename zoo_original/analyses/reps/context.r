   ...
   eqn Col (/Ast.Var(x)/, (ct, ce)) = (ct, ce)
     ...
     | Col (/Ast.CONS(e1,e2) as e/, (ct, ce)) =
       let val /ertg/ = ct /e/
           ...
           val pre = (ct + { /e1/ => e1rtg, /e2/ => e2rtg }, ce)
           val p1 = Col (/e1/, pre)
       in  Col (/e2/, p1)
       end
     | Col (/Ast.IF(e1,e2,e3) as e/, (ct, ce)) =
       let val /ertg/ = ct /e/
           ...
           val pre = (ct + { /e1/ => e1rtg, /e2/ => /ertg/, /e3/ => /ertg/ },
                      ce)
           val p1 = Col (/e1/, pre)
           val p2 = Col (/e2/, p1)
       in  Col (/e3/, p2)
     ...
     | Col (/Ast.CALL(f,es) as e/, (ct, ce)) =
	   ...
	   val xeset = /Ast.args(f,es)/
           ...
           fun post 1 = Col (/Ast.nth(es,1)/, pre)
             | post (i in range) = Col (/Ast.nth(es,i)/, post (i-1))
       in  post /Ast.len(es)/
       end
